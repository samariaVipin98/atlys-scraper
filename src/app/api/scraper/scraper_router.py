...
@router.post("/scraper")
def scrape_data(request: ScraperRequest, background_tasks: BackgroundTasks, db: SessionLocal = Depends(get_db), current_user: User = Depends(User.get_current_user)):
